...
export default function AdminOverviewPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if ('myOrgIds' in result && result.myOrgIds) setMyOrgIds(result.myOrgIds as unknown as string[]);
      if ('myProjectIds' in result && result.myProjectIds) setMyProjectIds(result.myProjectIds as unknown as string[]);
      if ('organizations' in result && result.organizations) setOrganizations(result.organizations as unknown as Organization[]);
      if ('projects' in result && result.projects) setProjects(result.projects as unknown as Project[]);
      if ('users' in result && result.users) setUsers(result.users as unknown as User[]);
    } catch (error) {
