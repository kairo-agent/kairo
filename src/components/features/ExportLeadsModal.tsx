...
function FloatingCalendar({
  ...
  // Build disabled matchers array (avoid undefined in object)
  const disabledMatchers: Array<{ before: Date } | { after: Date }> = [];
  if (disabledBefore) disabledMatchers.push({ before: disabledBefore });
  if (disabledAfter) disabledMatchers.push({ after: disabledAfter });
  ...
  useEffect(() => {
    function update() {
      ...
      if (mobile || !anchorRef.current) return;

      const rect = anchorRef.current.getBoundingClientRect();
      const calHeight = 360; // approximate calendar height
      const spaceBelow = window.innerHeight - rect.bottom;
      const direction = spaceBelow >= calHeight + 8 ? 'down' : 'up';

      setPos({
        top: direction === 'down' ? rect.bottom + 8 : rect.top - calHeight - 8,
  ...
  // Desktop: fixed position floating near the anchor
  return createPortal(
    <div
